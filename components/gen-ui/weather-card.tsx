    ...
    <div className="my-4 w-full max-w-full">
      <Card className={`overflow-hidden border-border/60 shadow-lg ${cardBg}`}>
        <CardHeader className="pb-3">
          <div className="flex items-start justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              {highLowText ? (
                <div className="rounded-full border bg-shade/40 px-3 py-1 text-sm font-medium text-fg-on-fill/90">
                  {highLowText}
                </div>
              ) : null}
              <div className="rounded-full border bg-shade/45 p-2 text-fg-on-fill/90">
        ...
        <CardContent className="space-y-3">
          <div className="rounded-2xl border weather-panel p-4 text-fg-on-fill">
            <div className="flex items-end justify-between gap-3">
              <div className="flex items-end gap-2">
                <Thermometer className="size-5 text-fg-on-fill/80" />
                <div>
                  <div className="text-5xl font-bold leading-none">{Math.round(temperature)}°C</div>
                  <div className="mt-1 text-sm text-fg-on-fill/75">
                    Feels like{" "}
                    <span className="font-semibold text-fg-on-fill">{Math.round(feelsLike)}°C</span>
                  </div>
                </div>
              </div>
              <div className="text-right text-sm text-fg-on-fill/80">
                <div>Wind</div>
                <div className="font-semibold text-fg-on-fill">{roundedWindMph} mph</div>
              </div>
            </div>
            ...
            {hourlyItems.length ? (
              <div className="mt-4">
                <div className="mb-2 flex items-center justify-between text-sm">
                  <h4 className="font-semibold text-fg-on-fill">Hourly forecast</h4>
                  <span className="text-fg-on-fill/70">Next {hourlyItems.length} hours</span>
                </div>
                <div className="flex gap-2 overflow-x-auto pb-2">
                  {hourlyItems.map((h, idx) => (
                    <div
                      key={`${h.time}-${idx}`}
                      className="min-w-[92px] shrink-0 rounded-xl border border-hairline bg-shade/45 px-2.5 py-2.5 text-center"
                    >
                      <div className="text-xs text-fg-on-fill/70 whitespace-nowrap">{h.time}</div>
                      <div className="mt-1 text-2xl font-semibold leading-none">
                        {Math.round(h.temperature)}°
                      </div>
                      <div className="mt-1 text-xs leading-tight text-fg-on-fill/70">
