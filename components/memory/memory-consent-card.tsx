    ...
    <Card className={cn("shrink-0 rounded-2xl", brandGlassCardClass)}>
      <CardContent className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex items-start gap-3.5">
          <span className="mt-0.5 flex size-10 shrink-0 items-center justify-center rounded-xl border border-white/10 bg-white/[0.04]">
            <Brain className="size-5 text-orange-300/85" aria-hidden />
          </span>
          <div>
            <div className="flex flex-wrap items-center gap-2">
              {/* `h2`: the page's `h1` is "Memory Center", so this is the next
                  level down and skipping to `h3` would break the outline. */}
              <h2 className="text-[15px] font-medium text-white">Long-term memory</h2>
              {consent.isPending ? (
        ...
        <div className="flex shrink-0 items-center gap-2">
          <Button
            ...
            className={cn(
              ...
              isOn
                ? "border border-white/12 bg-white/[0.06] text-zinc-200 hover:bg-white/[0.1]"
  ...
  return (
    <span
      className={cn(
        ...
        state === "granted"
          ? "border-orange-300/30 text-orange-200/90"
